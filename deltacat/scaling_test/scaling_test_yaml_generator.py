...
def replace_custom_resources(custom_resource_yaml_path, partition_values):
    ...
    for i in range(len(partition_values)):
        contents = single_custom_resource.replace(
            "{{partition_hash}}", f"partition_{i}"
        )
        custom_resource_str += f"\n{contents}"
    return custom_resource_str


def generate_scaling_test_yaml(
    yaml_file_path, custom_resource_yaml_path, partition_values
):
    custom_resource_str = replace_custom_resources(
        custom_resource_yaml_path, partition_values
    )
...
generate_scaling_test_yaml(
    "scaling_test_example.yaml", "one_custom_resource.yaml", [i for i in range(250)]
)
